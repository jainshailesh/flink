 ...
 * which emits detected pattern sequences as a map of events associated with
 * their names. The pattern is detected using a {@link
 * org.apache.flink.cep.nfa.NFA}. In order to process the detected sequences,
 * the user has to specify a {@link PatternSelectFunction} or a {@link
 * PatternFlatSelectFunction}.
 * <p>
 * <p>Additionally it allows to handle partially matched event patterns which
 * have timed out. For this the user has to specify a {@link
 * PatternTimeoutFunction} or a {@link PatternFlatTimeoutFunction}.
 *
...
public class PatternStream<T> {
    ...
    PatternStream(final DataStream<T> inputStream, final Pattern<T, ?>
            pattern, final EventComparator<T> comparator) {
        this.inputStream = inputStream;
        this.pattern = pattern;
     ...
     *
     * @param patternSelectFunction The pattern select function which is called
     *                              for each detected pattern sequence.
     * @param <R>                   Type of the resulting elements
     * @return {@link DataStream} which contains the resulting elements from
     * the pattern select function.
     */
    public <R> SingleOutputStreamOperator<R> select(final
                                                    PatternSelectFunction<T,
                                                            R>
                                                            patternSelectFunction) {
        // we have to extract the output type from the provided pattern
        // selection function manually
        // because the TypeExtractor cannot do that if the method is wrapped
        // in a MapFunction
    ...
    /**
     * Invokes the {@link org.apache.flink.api.java.ClosureCleaner} on the
     * given function if closure cleaning is enabled in the {@link
     * ExecutionConfig}.
     *
    ...
    public <R> SingleOutputStreamOperator<R> select(final
        ...
        if (this.useMixedTimeOperator) {
            Logger.getRootLogger().info("Creating a new pattern stream using" +
                    " mixed approach.");
            return CEPOperatorUtils.createPatternStreamMixedApproach
                    (inputStream,
                            pattern, comparator, clean
                                    (patternSelectFunction), outTypeInfo);
        }
        Logger.getRootLogger().info("Creating a new pattern stream using" +
                " normal approach.");
        return CEPOperatorUtils.createPatternStream(inputStream, pattern,
                comparator, clean(patternSelectFunction), outTypeInfo);
    }
     ...
     * The pattern select function can produce exactly one resulting element.
     * <p>
     * <p>Applies a timeout function to a partial pattern sequence which has
     * timed out. For each partial pattern sequence the provided {@link
     * PatternTimeoutFunction} is called. The pattern timeout function can
     ...
     * SingleOutputStreamOperator} resulting from the select operation with the
     * same {@link OutputTag}.
     *
     * @param timeoutOutputTag       {@link OutputTag} that identifies side
     *                               output with timed out patterns
     * @param patternTimeoutFunction The pattern timeout function which is
     *                               called for each partial pattern sequence
     *                               which has timed out.
     * @param patternSelectFunction  The pattern select function which is
     *                               called for each detected pattern
     *                               sequence.
     * @param <L>                    Type of the resulting timeout elements
     * @param <R>                    Type of the resulting elements
     * @return {@link DataStream} which contains the resulting elements with
     * the resulting timeout elements in a side output.
     */
    public <L, R> SingleOutputStreamOperator<R> select(
        ...
        TypeInformation<R> rightTypeInfo = TypeExtractor
                .getUnaryOperatorReturnType(
                patternSelectFunction,
                PatternSelectFunction.class,
                0,
     ...
     * @param <L>                    Type of the resulting timeout elements
     * @param <R>                    Type of the resulting elements
     * @return {@link DataStream} which contains the resulting elements or the
     * resulting timeout elements wrapped in an {@link Either} type.
     * @deprecated Use {@link PatternStream#select(OutputTag,
     * PatternTimeoutFunction, PatternSelectFunction)} that returns timed out
     * events as a side-output
     */
    @Deprecated
    public <L, R> SingleOutputStreamOperator<Either<L, R>> select(
        ...
        TypeInformation<L> leftTypeInfo = TypeExtractor
                .getUnaryOperatorReturnType(
                patternTimeoutFunction,
        ...
        final OutputTag<L> outputTag = new OutputTag<L>(UUID.randomUUID()
                .toString(), leftTypeInfo);

        final SingleOutputStreamOperator<R> mainStream = CEPOperatorUtils
        ...
        final DataStream<L> timedOutStream = mainStream.getSideOutput
                (outputTag);

        TypeInformation<Either<L, R>> outTypeInfo = new EitherTypeInfo<>
                (leftTypeInfo, rightTypeInfo);

        return mainStream.connect(timedOutStream).map(new CoMapTimeout<>())
                .returns(outTypeInfo);
    }

    /**
     * Applies a flat select function to the detected pattern sequence. For
     * each pattern sequence the provided {@link PatternFlatSelectFunction} is
     * called. The pattern flat select function can produce an arbitrary number
     * of resulting elements.
     *
     * @param patternFlatSelectFunction The pattern flat select function which
     *                                  is called for each detected pattern
     *                                  sequence.
     * @param <R>                       Type of the resulting elements
     * @return {@link DataStream} which contains the resulting elements from
     * the pattern flat select function.
     */
    public <R> SingleOutputStreamOperator<R> flatSelect(final
        ...
        // because the TypeExtractor cannot do that if the method is wrapped
        // in a MapFunction
        TypeInformation<R> outTypeInfo = TypeExtractor
                .getUnaryOperatorReturnType(
                patternFlatSelectFunction,
     ...
     * Applies a flat select function to the detected pattern sequence. For
     * each pattern sequence the provided {@link PatternFlatSelectFunction} is
     * called. The pattern select function can produce exactly one resulting
     * element.
     * <p>
     * <p>Applies a timeout function to a partial pattern sequence which has
     * timed out. For each partial pattern sequence the provided {@link
     * PatternFlatTimeoutFunction} is called. The pattern timeout function can
     * produce exactly one resulting element.
     * <p>
     * <p>You can get the stream of timed-out data resulting from the {@link
     * SingleOutputStreamOperator#getSideOutput(OutputTag)} on the {@link
     ...
     * @param timeoutOutputTag           {@link OutputTag} that identifies side
     *                                   output with timed out patterns
     * @param patternFlatTimeoutFunction The pattern timeout function which is
     *                                   called for each partial pattern
     *                                   sequence which has timed out.
     * @param patternFlatSelectFunction  The pattern select function which is
     *                                   called for each detected pattern
     *                                   sequence.
     * @param <L>                        Type of the resulting timeout
     *                                   elements
     * @param <R>                        Type of the resulting elements
     * @return {@link DataStream} which contains the resulting elements with
     * the resulting timeout elements in a side output.
     ...
     *                                   called for each detected pattern
     *                                   sequence.
     * @param outTypeInfo                Explicit specification of output
     *                                   type.
     * @param <L>                        Type of the resulting timeout
     *                                   elements
     * @param <R>                        Type of the resulting elements
     ...
     *                                   which is called for each partial
     ...
     * @return {@link DataStream} which contains the resulting events from the
     * pattern flat select function or the resulting timeout events from the
     * pattern flat timeout function wrapped in an {@link Either} type.
     * @deprecated Use {@link PatternStream#flatSelect(OutputTag,
     * PatternFlatTimeoutFunction, PatternFlatSelectFunction)} that returns
     * timed out events as a side-output
     */
    @Deprecated
